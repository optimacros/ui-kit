...
export class TabHeader extends Component<Props> {
    ...
    componentDidUpdate(prevProps: React.PropsWithChildren<Props>): void {
        ...
        if (this.props.draggable && this.dragTarget) {
            ...
            this.dragTarget.offsetLeft = (
                tabsScrollerNode?.children[this.dragTarget.index] as HTMLElement
            )?.offsetLeft;
        }
    }
    ...
    renderTabsHeaderButton(isScrollableTabs: boolean): (React.JSX.Element | null)[] | null {
        ...
        return map(tabs, (tab, index) => {
            ...
            const isDisabled = has(tab.props, 'disabled') ? tab.props.disabled : false;

            const styleContainer: CSSProperties = {};
            const style: CSSProperties = {};
            const isDraggableTab =
                !tab.props.nonDraggable && this.dragTarget && this.dragTarget.index === index;
            ...
            const tabButtonInnerProps: React.DetailedHTMLProps<
                React.HTMLAttributes<HTMLDivElement>,
                HTMLDivElement
            > = {
                className: theme.TabButton_Inner,
                style: style,
                onClick: isDisabled ? undefined : this.switchTab.bind(this, position),
                onMouseDown:
                ...
                onContextMenu: isDisabled
                    ...
                    : (event: React.MouseEvent<HTMLElement>) =>
                          this.onHeaderContextMenu(event, tab),
                onDoubleClick: isDisabled ? undefined : () => this.onDoubleClick(tab),
            };
    ...
    renderCounter(tab: React.JSX.Element): React.JSX.Element | null {
        const { counter, maxCounter, counterLink } = tab.props as TabProps;

        if (!counter) {
            return null;
        }

        return <Counter value={counter} maxValue={maxCounter} route={counterLink} />;
    }
    ...
    renderIcon = (tab: React.JSX.Element): React.JSX.Element | null => {
        ...
        if (typeof props.icon === 'string') {
            return <Icon className={styles.Icon} value={props.icon} />;
        }
