...
export default class TabHeaderState {
    constructor() {
        makeObservable(this)
    }
    ...
    @action scrollToTab(index: number, toRight = false) {
        ...
        if (index >= 0) {
            setTimeout(action(() => {
                ...
                const countScrolledTabs = _.findIndex(this._scrollableTabsOffsetsLeft, (offset) => {
        ...
        return _.reduce(
            ...
            (result, { current }, index) => {
                if (current && this.scrollableTabsChildren[index]) {
                    ...
                    const hiddenOnTheRight = this._scrollableTabsOffsetsLeft[index] + _.floor(tabWidth)
                        > _.ceil(tabsScrollerWidth) + this._scrollLeft
                    const hiddenOnTheLeft = this._scrollableTabsOffsetsLeft[index] < this._scrollLeft
